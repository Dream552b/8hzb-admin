...
import {
  ...
  updateTopStatus,
  updateIsClose
} from "../../api/competition/football";
import Pagination from "@/components/Pagination";
import MatchDataDialog from "./components/basketball/eventData.vue";
import LiveDataDialog from "./components/football/liveData.vue";
import EventIndexDialog from "./components/football/eventIndex.vue";
...
const columns = ref<any>([
  ...
  { label: "比赛名称", prop: "competitionName" },
  {
    ...
    formatter: (row, col, val) => {
      ...
      if (val == 3) return "已结束";
      if (val == 4) return "已结束";
      if (val < 1 || val >= 9) return "其他";
      if (val > 1 || val <= 7) return "进行中";
    }
  },
  {
    label: "比赛时间",
...
const getList = async () => {
  try {
    ...
    const res: any = await getFootballScheduleList(params);
    const {
      data: { records, total }
    } = res;
